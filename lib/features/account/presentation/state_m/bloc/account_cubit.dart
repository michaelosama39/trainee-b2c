...
import 'package:upgrade_traine_project/features/account/domain/usecase/create_feedback_usecase.dart';
...
class AccountCubit extends Cubit<AccountState> {
  ...
  void createFeedback(BuildContext context , CraeteFeedbackRequest body) async {
    emit(const AccountState.accountLoading());

    final result = await getIt<CreateFeedbackUseCase>()(body);

    result.pick(
      onData: (data) {
        Navigator.pop(context);
        emit(AccountState.createFeedback(result.data!));
      },
      onError: (error) {
        emit(AccountState.accountError(error, () => createFeedback(context, body)));
      },
    );
  }
