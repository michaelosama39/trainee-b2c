...
class MapsCubit extends Cubit<MapsState> {
  ...
  void emitPlaceSuggestion(String place, String sessionToken){
    placeSuggestationRepo.fetchPlaceSuggestationRepo(place, sessionToken).then((places){
  ...
  void emitPlaceLocation(String placeId, String sessionToken) {
    ...
    placeSuggestationRepo.getPlaceLocation(placeId, sessionToken).then((place) {
      controller.animateCamera(CameraUpdate.newCameraPosition(CameraPosition(target: LatLng(place.result.geometry.location.lat,place.result.geometry.location.lng,),zoom: 12)));
